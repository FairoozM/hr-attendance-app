...
import { fmtMoney, fmtPct, STORAGE_KEY_ROWS } from '../management/allPricesEcommerceUtils'
import {
  loadSavedCompositeItemsCustom,
  removeSavedCompositeItemCustom,
  SAVED_COMPOSITES_CUSTOM_UPDATED_EVENT,
  STORAGE_KEY_SAVED_COMPOSITES_CUSTOM,
} from './compositeBundlePricingUtils'

interface SavedCompositeEconomics {
  ok?: boolean
  salesPrice?: number
  vatAmount?: number
...
interface SavedCompositeComponent {
  item_id?: string
  sku?: string
  name?: string
  quantity?: number
  purchaseMatch?: { itemNo?: string; matchKind?: string } | null
  matchedListRow?: { shipping?: string | number; dateOfPrices?: string } | null
  matchedEconomics?: {
    denominatorInvalid?: boolean
    salesPrice?: number
    vatAmount?: number
    commissionAmount?: number
    advertisingAmount?: number
  ...
  purchaseFromList?: number | null
  lineTotal?: number | null
  missing?: boolean
}

interface SavedCompositeItem {
  sku: string
  name?: string
  composite_item_id?: string
  components?: SavedCompositeComponent[]
  rates?: {
    vatPct?: number
  ...
  bundleShipping?: number
  dateOfPrice?: string
  economics?: SavedCompositeEconomics
  updated_at?: string
}
...
function CompositeDetailTable({ item }: { item: SavedCompositeItem }) {
...
export function SavedCompositeItemsCustomPage() {
  ...
  useEffect(() => {
    const bump = (event: Event) => {
      ...
      if (
        event.type === SAVED_COMPOSITES_CUSTOM_UPDATED_EVENT ||
        event.type === 'focus' ||
        storageEvent.key === STORAGE_KEY_SAVED_COMPOSITES_CUSTOM ||
        storageEvent.key === STORAGE_KEY_ROWS
      ) {
        setSyncTick((t) => t + 1)
      }
    }
    window.addEventListener(SAVED_COMPOSITES_CUSTOM_UPDATED_EVENT, bump)
    window.addEventListener('storage', bump)
    window.addEventListener('focus', bump)
    return () => {
      window.removeEventListener(SAVED_COMPOSITES_CUSTOM_UPDATED_EVENT, bump)
  ...
  const savedItems = useMemo(() => {
    void syncTick
    return loadSavedCompositeItemsCustom() as SavedCompositeItem[]
  }, [syncTick])
  ...
  const toggleExpanded = useCallback((sku: string) => {
    setExpanded((prev) => {
  ...
  const handleRemove = useCallback((sku: string) => {
    if (!window.confirm(`Remove saved custom composite item ${sku}?`)) return
    removeSavedCompositeItemCustom(sku)
    setExpanded((prev) => {
      const next = new Set(prev)
      next.delete(sku)
      return next
  ...
  return (
    <div className="page composite-prices-page ap-ec-page">
      <div className="doc-page-hero">
        <div>
          <h1 className="doc-page-title">Saved Composite Items (Custom)</h1>
          <p className="doc-page-subtitle">
            Composites saved from{' '}
            <NavLink to="/prices/composite-items-custom">Composite Items Prices (Custom)</NavLink>. Separate from the
            standard Saved Composite Items list.
          </p>
        </div>
      </div>

      <section className="page-section cb-bundle-section" aria-label="Saved custom composite item SKUs">
        <div className="cb-bundle-toolbar">
          <NavLink className="btn btn--primary" to="/prices/composite-items-custom">
            + Add / fetch composite
          </NavLink>
          ...
          <label className="cb-saved-search">
            ...
            <input
              ...
              placeholder="Search saved custom composites..."
              autoComplete="off"
            />
          </label>
        </div>
        ...
        {savedItems.length === 0 ? (
          <p className="composite-prices-placeholder">
            No custom composite SKUs saved yet. Open{' '}
            <NavLink to="/prices/composite-items-custom">Composite Items Prices (Custom)</NavLink>, fetch a composite
            item, then click <strong>Save composite item</strong>.
          </p>
        ) : filteredItems.length === 0 ? (
          <p className="composite-prices-placeholder">
            No saved custom composite items match <strong>{searchText}</strong>.
          </p>
        ) : (
          <div className="ap-table-scroll cb-table-scroll">
            <table className="ap-ec-table cb-saved-table">
              <thead>
                <tr>
                  <th scope="col" className="cb-saved-table__toggle">
                    +
                  </th>
                  ...
                  return (
                    <Fragment key={item.sku}>
                      <tr key={`${item.sku}-summary`} className="cb-saved-table__summary-row">
                        ...
                        <td>{fmtMoney(item.bundleShipping, 2)}</td>
                        <td className="cb-sales-price-cell">
                          {economics.ok ? fmtMoney(economics.salesPrice, 0) : '—'}
                        </td>
                        <td>{economics.ok ? fmtMoney(economics.totalCost, 2) : '—'}</td>
                        <td>{economics.ok ? fmtMoney(economics.profit, 2) : '—'}</td>
                        <td className="cb-profit-percent-cell">
