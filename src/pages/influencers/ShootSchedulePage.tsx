...
function shootBadge(status: string) {
  const map: Record<string, string> = {
    Scheduled: 'inf-badge--scheduled',
    Confirmed: 'inf-badge--shortlisted',
    Completed: 'inf-badge--completed',
    Cancelled: 'inf-badge--rejected',
...
function ShootCard({ inf, onOpen, onMarkDone }: ShootCardProps) {
  return (
    <div className="inf-schedule-card" onClick={() => onOpen(inf.id)}>
      ...
      <div className="inf-schedule-card__meta">
        {inf.shootLocation ? <span>📍 {inf.shootLocation}</span> : null}
        {inf.campaign ? <span>🎯 {inf.campaign}</span> : null}
        {inf.assignedTo ? <span>👤 {inf.assignedTo}</span> : null}
      </div>
      <div style={{ marginTop: '0.6rem', display: 'flex', gap: '0.35rem', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: '0.3rem' }}>
          <button
            ...
            onClick={(e) => { e.stopPropagation(); onMarkDone(inf.id) }}
          >
            ✓ Done
          </button>
        </div>
      ...
      <div className="inf-toolbar" style={{ marginBottom: '1.25rem' }}>
        <div className="inf-filter-chips">
          {['All', 'Shoot Scheduled', 'Shot Completed', 'Waiting for Upload', 'Uploaded'].map((s) => (
            <button key={s} type="button" className={`inf-chip ${filterStatus === s ? 'inf-chip--active' : ''}`} onClick={() => setFilterStatus(s)}>
              {s}
            </button>
          ))}
        </div>
              ...
              <div className="inf-schedule-grid" style={{ marginBottom: '1.5rem' }}>
                {upcoming.map((inf) => (
                  <ShootCard
                    key={inf.id}
                    inf={inf}
                    onOpen={(id) => navigate(`/influencers/${id}/edit`)}
                    onMarkDone={(id) => updateInfluencer(id, { workflowStatus: 'Shot Completed' })}
                  />
                ))}
              </div>
            </>
                ...
                <tr key={inf.id} onClick={() => navigate(`/influencers/${inf.id}/edit`)}>
                  ...
                  <td>
                    <div className="inf-table__actions" onClick={(e) => e.stopPropagation()}>
                      <button
                        type="button"
                        className="inf-btn inf-btn--success inf-btn--xs"
