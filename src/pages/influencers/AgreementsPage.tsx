...
type AgreementFilterStatus = 'All' | 'Pending' | 'Generated' | 'Signed'

function AgreementPreview({ inf, onClose }: { inf: Influencer; onClose: () => void }) {
...
export function AgreementsPage() {
  ...
  const [previewInf, setPreviewInf] = useState<Influencer | null>(() =>
    previewId ? influencers.find((i) => i.id === previewId) ?? null : null,
  )
  const [filterStatus, setFilterStatus] = useState<AgreementFilterStatus>('All')
  ...
  const generateAgreement = (inf: Influencer) => {
    ...
    <div className="inf-page">
      <div className="inf-page-header">
        <div>
          ...
          <p className="inf-page-subtitle">Legal agreement workflow for roster influencers — separate from performance contract check-ins.</p>
        </div>
      </div>
      ...
      <div className="inf-toolbar">
        ...
        <div className="inf-filter-chips">
          {(['All', 'Pending', 'Generated', 'Signed'] as const).map((s) => (
