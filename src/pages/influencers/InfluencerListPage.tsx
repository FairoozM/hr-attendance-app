...
function InfluencerListAvatar({ name, imageUrl }: { name: string; imageUrl: string }) {
...
function InstagramCell({ handle, url }: { handle?: string; url?: string }) {
...
function parseFollowersCount(value: string | number | null | undefined): number {
...
function formatFollowersCell(value: string | number | null | undefined): string {
...
type FollowerFilter = (typeof FOLLOWER_FILTER_OPTIONS)[number]['value']

function matchesFollowerFilter(count: string | number | null | undefined, filter: FollowerFilter): boolean {
...
type QuickChipValue = (typeof QUICK_CHIP)[keyof typeof QUICK_CHIP]

type QuickAction = 'approve' | 'reject' | 'schedule' | 'agreement' | 'payment-ready' | 'paid' | 'delete'
...
type ListColKey = (typeof LIST_COL_KEYS)[number]

type ColWidths = Record<ListColKey, number>

const DEFAULT_COL_WIDTHS: ColWidths = Object.freeze({
  sr: 46,
  name: 240,
  nationality: 86,
  ig: 150,
...
function ResizableTh({
  ...
  className?: string
  children: ReactNode
  onResizeStart: (e: MouseEvent, index: number) => void
}) {
  return (
...
function InfluencerRowActions({
  ...
  onQuickAction: (e: MouseEvent, action: QuickAction, inf: Influencer) => void
}) {
  ...
  const run = (e: MouseEvent, fn: (ev: MouseEvent) => void) => {
    e.stopPropagation()
    setOpen(false)
    fn(e)
  }
...
export function InfluencerListPage() {
  ...
  const [filterFollowers, setFilterFollowers] = useState<FollowerFilter>('All')
  const [quickChip, setQuickChip] = useState<QuickChipValue>(QUICK_CHIP.ALL)
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === 'newest') list = [...list].sort((a, b) => Number(new Date(b.createdAt)) - Number(new Date(a.createdAt)))
    else if (sortBy === 'updated') list = [...list].sort((a, b) => Number(new Date(b.updatedAt)) - Number(new Date(a.updatedAt)))
  ...
  const startColResize = useCallback((e: MouseEvent, index: number) => {
    e.preventDefault()
    e.stopPropagation()
  ...
  const handleQuickAction = (e: MouseEvent, action: QuickAction, inf: Influencer) => {
    e.stopPropagation()
  ...
  return (
    <div className="inf-page inf-page--list">
      ...
      <div className="inf-page-header">
        <div>
          <h1 className="inf-page-title">Influencer Roster</h1>
          <p className="inf-page-subtitle">
            {(listMeta && !listMeta.isFullListClientPaging ? listMeta.total : influencers.length)} influencers
            {' · '}CRM master records — approval, contact, packages, and workflow. Performance contracts live under Performance Contracts.
          </p>
        </div>
      ...
      <div className="inf-toolbar">
        ...
        <select className="inf-select" value={filterFollowers} onChange={(e) => { setQuickChip(QUICK_CHIP.ALL); setFilterFollowers(e.target.value as FollowerFilter) }}>
          {FOLLOWER_FILTER_OPTIONS.map((o) => (
            <option key={o.value} value={o.value}>{o.label}</option>
          ))}
        </select>
        <select className="inf-select" value={sortBy} onChange={e => setSortBy(e.target.value as SortOption)}>
