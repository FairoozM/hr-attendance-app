...
import type { InfluencersContextValue } from '../../contexts/InfluencersContext'

export const MAX_INSIGHT_IMAGES = 6

export interface InsightsImagesSectionProps {
  influencerId: string
  imageKeys: string[]
...
export function InsightsImagesSection({
  ...
  const [signedByKey, setSignedByKey] = useState<Record<string, string>>({})
  ...
  const persistKeys = useCallback(
    async (newKeys: string[], rotationsPatch?: Record<string, number>) => {
      if (!influencerId) return
      const r =
        rotationsPatch != null
          ? rotationsPatch
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          setSignedByKey({})
          setLoadError(e instanceof Error ? e.message : 'Could not load images')
        }
      } finally {
  ...
  const openPicker = () => {
    ...
    try {
      ...
      setUploadLine(`Could not open file picker: ${err instanceof Error ? err.message : String(err)}`)
    }
  }

  const onDeleteOne = async (removeKey: string) => {
    if (!canEdit || busy) {
      return
    }
  ...
  const onDeleteAll = async () => {
    ...
    try {
      ...
      window.alert(err instanceof Error ? err.message : 'Failed to clear images')
    } finally {
      setBusy(false)
    }
  }

  const onRotate = async (k: string) => {
    if (!canEdit || busy) {
      return
    }
    ...
    try {
      ...
      window.alert(e instanceof Error ? e.message : 'Could not save rotation')
    }
  }
  ...
  const onKeyDragStart = (e: DragEvent, key: string) => {
    if (!canEdit) {
      return
    }
  ...
  const onKeyDragOver = (e: DragEvent) => {
    e.preventDefault()
  ...
  const onKeyDropOnCell = async (e: DragEvent, targetKey: string) => {
    e.preventDefault()
    e.stopPropagation()
    if (e.dataTransfer?.files?.length) {
      setIsDragging(false)
    ...
    try {
      ...
      window.alert(err instanceof Error ? err.message : 'Reorder failed')
    } finally {
      setBusy(false)
    }
  }

  const previewKey = (k: string) => {
