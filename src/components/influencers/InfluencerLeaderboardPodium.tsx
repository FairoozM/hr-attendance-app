...
function getDatasetBestsFromContracts(contracts: InfluencerContract[]): DatasetBests | null {
  ...
  contracts.forEach((c) => {
    const rec: Partial<InfluencerPerformance> = c.latest || {}
...
type MiniStatProps = {
  label: string
  value: string
  isBest: boolean | null | undefined
  variant: string
}

function MiniStat({ label, value, isBest, variant }: MiniStatProps) {
  return (
...
type PodiumCardProps = {
  ...
  tier: string
  bests: DatasetBests | null
  onSelect?: (contract: InfluencerContract) => void
}
...
export type InfluencerLeaderboardPodiumProps = {
  videoContracts: InfluencerContract[]
  rankingsByContractId: Map<string, InfluencerContractRanking>
  onSelectContract?: (contract: InfluencerContract) => void
}
...
export function InfluencerLeaderboardPodium({
  ...
  onSelectContract,
}: InfluencerLeaderboardPodiumProps) {
  ...
  const sorted = [...videoContracts]
    ...
    .filter((x): x is { contract: InfluencerContract; rank: InfluencerContractRanking } => Boolean(x.rank))
